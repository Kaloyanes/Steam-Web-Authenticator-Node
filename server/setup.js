...
function generateSdaDeviceId(steamid) {
    ...
    return new Promise((resolve, reject) => {
        ...
        community.login(loginDetails, (err, sessionID, cookies, steamguard, oAuthToken) => {
            ...
            PENDING_SETUPS.set(setupId, {
                ...
                cookies,
                sessionID: setupId,
                deviceID: generateSdaDeviceId(steamID64),
    ...
    return new Promise((resolve, reject) => {
        ...
        session.store.addPhoneNumber(number, false, (err) => {
            if (err) {
                ...
                if (
                    msg.includes('already has a phone number') ||
                    msg.includes('already has a phone')
                ) {
        ...
        session.community.enableTwoFactor((err, response) => {
            if (err) {
                if (err.eresult === SteamCommunity.EResult.RateLimitExceeded) {
                    ...
                    return reject(
                        new Error(
                            'Steam is rate-limiting authenticator setup for this account/IP (EResult 84). ' +
                            'Wait a while and try again, or use a different account.'
                        )
                    );
                }
